 ...
 * Protégé par ADMIN_SECRET ou stripe webhook.
 */
...
export async function POST(request) {
  // Auth : soit ADMIN_SECRET, soit appel interne depuis le webhook Stripe
  ...
  const { name, slug, type, plan, adminUserId, adminEmail, stripeCustomerId, stripeSubscriptionId } = body;

  if (!name || !slug || !adminUserId || !adminEmail) {
    return Response.json({ error: "Champs manquants : name, slug, adminUserId, adminEmail." }, { status: 400 });
  }
