...
export default function ResetPasswordPage() {
  ...
  async function handleSubmit(e) {
    ...
    if (error) {
      setError("Erreur lors de la réinitialisation. Recommence depuis l'email.");
      setLoading(false);
    } else {
      setSuccess(true);
    ...
    <div style={{ minHeight: "100vh", background: "#f8faff", display: "flex", flexDirection: "column", fontFamily: "'Outfit', system-ui, sans-serif" }}>
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 20px" }}>
        <div style={{ width: "100%", maxWidth: 420, background: "#fff", borderRadius: 20, padding: "40px 36px", boxShadow: "0 4px 32px rgba(15,23,42,0.08)", border: "1px solid #e2e8f0" }}>
          ...
          <p style={{ fontSize: 14, color: "#64748b", marginBottom: 28 }}>Choisis un nouveau mot de passe pour ton compte.</p>
          <form onSubmit={handleSubmit}>
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ position: "relative" }}>
                <input type={showPwd ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} required autoFocus
