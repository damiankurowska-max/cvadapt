...
export default function SignUpPage() {
  ...
  async function handleSignUp(e) {
    ...
    if (error) {
      setError(error.message.includes("already") ? "Cet email est déjà utilisé. Connecte-toi." : error.message);
      setLoading(false);
    } else {
      setSuccess(true);
  ...
  if (success) return (
    <div style={{ minHeight: "100vh", background: "#f8faff", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Outfit', system-ui, sans-serif" }}>
      <div style={{ textAlign: "center" }}>
        ...
        <h2 style={{ fontSize: 22, fontWeight: 800, color: "#0f172a", marginBottom: 8 }}>Compte créé !</h2>
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 20px" }}>
        <div style={{ width: "100%", maxWidth: 420, background: "#fff", borderRadius: 20, padding: "40px 36px", boxShadow: "0 4px 32px rgba(15,23,42,0.08)", border: "1px solid #e2e8f0" }}>
          <div style={{ marginBottom: 20 }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: "#2563eb", background: "#eff6ff", padding: "4px 12px", borderRadius: 999 }}>1 CV gratuit · Sans CB</span>
          </div>
          <h1 style={{ fontSize: 26, fontWeight: 800, color: "#0f172a", marginBottom: 8, letterSpacing: "-0.5px" }}>Créer un compte</h1>
          <p style={{ fontSize: 14, color: "#64748b", marginBottom: 28 }}>Génère 1 CV optimisé gratuitement</p>
          ...
          <form onSubmit={handleSignUp}>
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#374151", marginBottom: 6 }}>Mot de passe <span style={{ color: "#94a3b8", fontWeight: 400 }}>(min. 6 caractères)</span></label>
